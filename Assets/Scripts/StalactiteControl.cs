...
public class StalactiteControl : MonoBehaviour
{
    ...
    {
	if (collider.tag == "Player" && Physics2D.Linecast(base.transform.position, collider.transform.position, LayerMask.GetMask("Terrain")).collider == null)
	{
	    base.StartCoroutine(Fall(fallDelay));
	    trigger.OnTriggerEntered -= HandleTriggerEnter;
