	...
	void DrawLayoutOutline(Transform t) {
		...
		if (layout != null) {
			...
			if (sizer != null) {
				...
				if (sizer.horizontal) {
					Handles.ArrowHandleCap(0, (p[0] + p[3]) * 0.5f, Quaternion.LookRotation(p[1] - p[0]), arrowSize,EventType.MouseMove);
					Handles.ArrowHandleCap(0, (p[1] + p[2]) * 0.5f, Quaternion.LookRotation(p[0] - p[1]),  arrowSize, EventType.MouseMove);
				} else {
					Handles.ArrowHandleCap(0, (p[0] + p[1]) * 0.5f, Quaternion.LookRotation(p[3] - p[0]), arrowSize, EventType.MouseMove);
					Handles.ArrowHandleCap(0, (p[2] + p[3]) * 0.5f, Quaternion.LookRotation(p[0] - p[3]), arrowSize, EventType.MouseMove);
				}
			}
		}
